...
Future<void> enterBoxSeatMode(FakeWebSocketChannel channel) => sendAndPump(
  channel,
...
Future<void> endBootstrap(FakeWebSocketChannel channel, {bool inJam = false}) =>
    sendAndPump(channel, {'ScoreBoard.CurrentGame.InJam': inJam});
...
void main() {
  ...
  group('Group A — CRG message parsing', () {
    ...
    test(
      'Skater(uuid).RosterNumber populates roster and known numbers',
      () async {
        final (:engine, :channel, :state) = await setupEngine();
    ...
    test('Skater(uuid).Role=Jammer sets jammer seat number', () async {
      ...
      await sendAndPump(channel, {
        'ScoreBoard.CurrentGame.Team(1).Skater(uuid-j).RosterNumber': '77',
      });
    ...
    test('Unrecognized path is silently ignored', () async {
      ...
      // Should not throw
      await sendAndPump(channel, {'ScoreBoard.SomeUnknown.Path': 'value'});
    });
  });
  ...
  group('Group B — BoxClock bootstrap and ownership', () {
    ...
    test(
      'after bootstrap: owned seat, small delta (<1s) → NOT applied',
      () async {
        final (:engine, :channel, :state) = await setupEngine();
        await enterBoxSeatMode(channel);
    ...
    test(
      'after bootstrap: owned seat, large delta (>1s) → applied (server correction)',
      () async {
        final (:engine, :channel, :state) = await setupEngine();
        await enterBoxSeatMode(channel);
        await endBootstrap(channel);
    ...
    test(
      'after bootstrap: non-owned seat, any delta → always applied',
      () async {
        final (:engine, :channel, :state) = await setupEngine(
          role: AppRole.team1Full,
          teamIndex: 1,
        ...
        expect(
          ...
          const Duration(milliseconds: 24800),
        );
      },
    );

    test(
      'BoxClock.Running=false pauses owned seat (another controller stopped it)',
      () async {
        final (:engine, :channel, :state) = await setupEngine();
        await enterBoxSeatMode(channel);
    ...
    test(
      'BoxClock.Running=true applied as catch-up for occupied-but-stopped owned seat',
      () async {
        final (:engine, :channel, :state) = await setupEngine();
        await enterBoxSeatMode(channel);
        await endBootstrap(channel);
  ...
  group('Group D — WS commands for owned seats (PBM)', () {
    ...
    test(
      'seatSkater on jammer does NOT send BoxSkater (jammer auto-populated via role)',
      () async {
        fixture.state.seatSkater(
          seat: fixture.state.team1Jammer,
          number: '10',
  ...
  group('Group F — Reconnect behavior', () {
    test(
      'reconnect re-enters bootstrap phase — BoxClock.Time applied unconditionally',
      () async {
        final (:engine, :channel, :state) = await setupEngine();
        await enterBoxSeatMode(channel);
        await endBootstrap(channel);
