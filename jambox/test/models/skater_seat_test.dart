...
void main() {
  ...
  group('state getter', () {
    ...
    test('time at zero → done, regardless of isRunning', () {
      final runningAtZero = occupiedSeat(
          timeRemaining: Duration.zero, isRunning: true);
      final pausedAtZero = occupiedSeat(
          timeRemaining: Duration.zero, isRunning: false);
  ...
  group('setSkater()', () {
    ...
    test('arrivedBetweenJams reflects arrivedBetween argument', () {
      final seat = emptySeat();
      seat.setSkater(number: '1', pos: SkaterPosition.blocker, arrivedBetween: true);
      expect(seat.arrivedBetweenJams, isTrue);

      seat.setSkater(number: '2', pos: SkaterPosition.blocker, arrivedBetween: false);
