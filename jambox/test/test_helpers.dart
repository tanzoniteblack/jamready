...
class FakeWebSocketSink implements WebSocketSink {
  ...
  @override
  void add(dynamic data) => sent.add(jsonDecode(data as String) as Map<String, dynamic>);
...
class FakeWebSocketChannel extends StreamChannelMixin<dynamic>
  ...
  List<Map<String, dynamic>> sentSets(String keyFragment) => sent
      .where((m) => m['action'] == 'Set' && (m['key'] as String).contains(keyFragment))
      .toList();
...
SkaterSeat seatBlocker(
  ...
  state.seatSkater(seat: seat, number: number, position: SkaterPosition.blocker);
  return seat;
}
...
void expectJammerTimes(
  ...
  expect(state.team1Jammer.timeRemaining, team1,
      reason: 'T1 jammer time mismatch');
  expect(state.team2Jammer.timeRemaining, team2,
