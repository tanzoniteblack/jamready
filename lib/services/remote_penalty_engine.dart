...
class RemotePenaltyEngine extends PenaltyEngine with WidgetsBindingObserver {
  ...
  static final _reTeamName = RegExp(
    r'ScoreBoard\.CurrentGame\.Team\((\d)\)\.Name$',
  );
  static final _reTeamColor = RegExp(
    r'ScoreBoard\.CurrentGame\.Team\((\d)\)\.Color\((operator|penalty)\.(fg|bg|glow)\)$',
  );
  static final _reSkaterNumber = RegExp(
    r'ScoreBoard\.CurrentGame\.Team\((\d)\)\.Skater\(([^)]+)\)\.RosterNumber$',
  );
  ...
  _WsDelta _buildDelta(Map<String, dynamic> stateMap) {
    ...
    for (final entry in stateMap.entries) {
      ...
      if (key == 'ScoreBoard.CurrentGame.Clock(Jam).Running') {
        ...
        if (!delta.jamRunningIsDefinitive) {
          delta.jamRunning = value == true || value == 'true';
        }
        continue;
      }
      ...
      if (teamNameMatch != null) {
        delta.teamNames[int.parse(teamNameMatch.group(1)!)] =
            value?.toString() ?? '';
        continue;
      }
      ...
      if (skaterMatch != null) {
        ...
        if (number.isNotEmpty) {
          (delta.rosterNumbers[t] ??= {})[skaterMatch.group(2)!] = number;
        }
        continue;
      }
      ...
      if (roleMatch != null) {
        delta.skaterRoles.add((
          int.parse(roleMatch.group(1)!),
          roleMatch.group(2)!,
          value?.toString() ?? '',
        ));
        continue;
      ...
      if (legacySkaterMatch != null && value is Map) {
        delta.legacyRosters.add((
          int.parse(legacySkaterMatch.group(1)!),
          value,
        ));
        continue;
      }
  ...
  void _applyDelta(_WsDelta delta) {
    ...
    for (final e in delta.teamNames.entries) {
      _state.updateTeam(e.key, name: e.value);
    }
    for (final e in delta.teamColors.entries) {
      e.value.forEach(
        (key, color) => _state.updateTeamColor(e.key, key.$1, key.$2, color),
      );
    }

    for (final e in delta.rosterNumbers.entries) {
      e.value.forEach(
        (uuid, number) => _state.updateRoster(e.key, number, uuid),
      );
    }
    for (final (t, data) in delta.legacyRosters) {
      data.forEach((skaterId, skaterData) {
        if (skaterData is Map) {
          ...
          if (number.isNotEmpty) {
            _state.updateRoster(t, number, skaterId.toString());
          }
        }
      });
    }
  ...
  void _scheduleReconnect() {
    ...
    final delay =
        Duration(seconds: cappedSec) + Duration(milliseconds: jitterMs);
  ...
  void reportPenalty({
    ...
    if (skaterId == null) {
      _log.w(
        'Cannot report penalty: skater #$skaterNumber not found in roster for team $teamIndex',
      );
      return;
    }
